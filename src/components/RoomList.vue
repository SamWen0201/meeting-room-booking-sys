...
function deleteRoomItem(id: string): void {
  ...
  if (roomIsUsing(id)) {
    console.log(
      "Room is using or will use in the future. You can't delete the room. ",
    );
...
function roomIsUsing(roomId: string): boolean {
  ...
  const room = bookingListStore.bookings.find(
    (el) => el.roomId === roomId && el.endTime > now,
  );
...
<template>
  <div class="roomlist">
    <el-row justify="space-between">
      ...
      <el-button
        type="primary"
        color="#3B82F6"
        @click="dialogRoomFormVisible = true"
      >
      ...
      <el-table-column label="操作" width="180">
        <template #default="scope">
          <el-button
            ...
            type="primary"
            color="#3B82F6"
            @click="editRoomItem(scope.row.id)"
          >
            <span class="u-margin-right-sm u-flex-center">
          ...
          <el-button
            ...
            type="danger"
            color="#EF4444"
            @click="deleteRoomItem(scope.row.id)"
          >
            <span class="u-margin-right-sm u-flex-center">
  ...
  >
    <RoomForm
      @closeDialoagRoomForm="handleCloseDialoagRoomForm"
      :editData="roomEditData"
    ></RoomForm>
  </el-dialog>
</template>
