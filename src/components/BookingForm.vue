...
function combineDateTime(): Date[] {
    ...
    const [startHour, startMinute] = startTime.value.split(":").map(Number) as [number, number];
    const [endHour, endMinute] = endTime.value.split(":").map(Number) as [number, number];
...
function roomIsUsing(roomId: string): boolean {
    ...
    const bookingsInTheRoom: Booking[] = useBookingListStore.bookings.filter( (el): boolean => el.roomId === roomId);
    
    // 在使用該會議室的 多個預約記錄，找到跟 BookingForm 選擇的會議時間段 衝突的紀錄 
    const [bookingStartTime, bookingEndTime] = combineDateTime();
    
    if (bookingStartTime === undefined || bookingEndTime === undefined) { // 這邊只是先暫時處理，之後需要處理 bookingStartTime, bookingEndTime 可能會是 undefined 的問題
        console.log("booking form is not completely");
        return false;
    }
    // 
    const conflictBooking = bookingsInTheRoom.find( (el) => bookingStartTime.getTime() < el.endTime && bookingEndTime.getTime() > el.startTime);
